...
while [[ $# -gt 0 ]]; do
    case "$1" in
        -f|--force-source)
            ...
            ;;
        -h|--help)
            info "Usage: $(basename "$0") [--force-source]"
            exit 0
            ;;
        *)
...
success "glibc version: $glibc_version"
...
if ((glibc_num >= required_glibc_num)) && (( FORCE_SOURCE == 0 )); then
    step "Installing the latest yazi (linux, x86_64, gnu)"
    ...
    spinner $pid
    if ! wait $pid; then
        error "Download failed. Please ensure curl can access GitHub and try again."
        exit 1
    fi
    ...
    # Copy binaries
    info "Installing binaries to $XDG_PREFIX_HOME/bin/"
    ...
    success "yazi installed from pre-built binary"
else
    if (( FORCE_SOURCE )); then
        warning "Force source build requested; building from source (this may take several minutes)"
    else
        warning "glibc version < $REQUIRED_GLIBC, building from source (this may take several minutes)"
    fi

    # Check if cargo is installed
    if ! command -v cargo &> /dev/null; then
        error "cargo (Rust) is not installed. Please install Rust first:"
        info "Visit https://rustup.rs/ or run: curl --proto '=https' --tlsv1.2 -sSf https://sh.rustup.rs | sh"
        exit 1
    fi
    ...
    step "Detecting latest version tag"
    ...
    if [ -z "$LATEST_TAG" ]; then
        warning "Could not determine latest tag; using default branch"
    else
        info "Latest tag: $LATEST_TAG"
        if ! git checkout --quiet "$LATEST_TAG"; then
            warning "Checkout of $LATEST_TAG failed; staying on default branch"
        else
    ...
    # Build yazi
    step "Building yazi (this may take several minutes)"
    info "Running: cargo build --release --locked"
    cargo build --release --locked >/dev/null 2>&1 &
    pid=$!
    spinner $pid
    if ! wait $pid; then
        error "Build failed. Check your Rust toolchain and dependencies."
        exit 1
    fi
    success "Build completed"

    # Install binaries
    step "Installing binaries to $XDG_PREFIX_HOME/bin/"
    installed_any=0
    ...
    if [ "$installed_any" -eq 0 ]; then
        error "No expected binaries found in target/release"
        popd >/dev/null
        exit 1
    fi
    ...
    # Cleanup
    popd >/dev/null
    info "Build directory will be cleaned up"

    success "yazi built and installed from source"
fi
