...
installed standalone by `get.chezmoi.io`, and the Pixi lifecycle hook installs pixi
...
happening. The `kitty-terminfo` resource in the core bundle installs upstream's
compiled entry in both ncurses layouts, even on headless machines. The reason
for those two copies follows.
